...
export default function SavedView({ translation, onOpen }) {
  ...
  // Nota în curs de editare (cheia versetului) + textul tastat.
  ...
  function startEditNote(r) {
    setEditKey(r.key);
    setEditDraft(r.noteText);
  }

  // Salvează nota editată (text gol = ștergere, ca în cititor).
  function saveEditNote() {
    setNote(editKey, editDraft);
    setEditKey(null);
    setEditDraft('');
  }
                ...
                <li key={r.key} className={CARD}>
                  <div className="mb-1 flex items-center justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <IconButton label="Șterge semnul de carte" danger onClick={() => toggleBookmark(r.key)}>
                        <TrashIcon />
                      </IconButton>
                    </div>
                  </div>
              ...
              return (
                <li key={r.key} className={CARD}>
                  <div className="mb-1 flex items-center justify-between gap-3">
                    ...
                    <IconButton label="Șterge evidențierea" danger onClick={() => setHighlight(r.key, null)}>
                      <TrashIcon />
                    </IconButton>
                  </div>
                  <p className="line-clamp-2 leading-relaxed text-slate-700 dark:text-slate-300">{r.text}</p>
            ...
            {noteRows.map((r) => (
              <li key={r.key} className={CARD}>
                <div className="mb-1 flex items-center justify-between gap-3">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="text-xs text-slate-400 dark:text-slate-500">{fmtDate(r.at)}</span>
                    <IconButton label="Editează nota" onClick={() => startEditNote(r)}>
                      <PencilIcon />
                    </IconButton>
                    <IconButton label="Șterge nota" danger onClick={() => setNote(r.key, '')}>
                      <TrashIcon />
                    </IconButton>
                  </div>
                </div>
                ...
                {editKey === r.key ? (
                  <div className="mt-2 space-y-2">
                    <textarea
                      ...
                      autoFocus
                      aria-label={`Editează nota pentru ${r.ref}`}
...
function IconButton({ label, onClick, danger = false, children }) {
  return (
    <button
      ...
      aria-label={label}
      className={
        'rounded-md p-1.5 text-slate-400 transition hover:bg-slate-100 dark:text-slate-500 dark:hover:bg-slate-800 ' +
        (danger
